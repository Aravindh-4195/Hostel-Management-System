...
<html lang="en">
  ...
  <?php
    if (session_status() === PHP_SESSION_NONE) {
     ...
     include '../dbconnect.php';
        ...
        $connect=mysqli_query($conn,"select  * from local_outings where studentid='$regno' and status='0'");
            $rows=mysqli_fetch_assoc($connect);
        $result=mysqli_query($conn,"select * from non_local where Studentid='$regno' and status='0'");
    ...
    <?php 
        if((mysqli_num_rows($connect)==NULL) && mysqli_fetch_assoc($result)==NULL){
            $result=mysqli_query($conn,"insert into local_outings (`studentid`) values('$regno')");
            $connect=mysqli_query($conn,"select  * from local_outings where studentid='$regno'");
            $rows=mysqli_fetch_assoc($connect);
            ?>
  ...
  <div class="heading" >
    ...
    <?php }
        ...
        header("Refresh:0.01;url=presentOuting.php",true,303);
    }?>
